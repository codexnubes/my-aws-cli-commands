...
aws --profile adt-prod emr --output "json" --region us-east-1 create-cluster \
--ami-version 3.7 \
...
--ec2-attributes "InstanceProfile=EMR_EC2_DefaultRole,KeyName=adt-shared-prod,AvailabilityZone=${SPOT_AVAILABILITY_ZONE}" \
--instance-groups \
...
--bootstrap-actions \
  ...
  Name=ConfigureSpark,Path=s3://support.elasticmapreduce/spark/configure-spark.bash,Args=["spark.executor.memory=20g","spark.driver.memory=2g","spark.local.dir=/mnt/spark,/mnt1/spark"] \
